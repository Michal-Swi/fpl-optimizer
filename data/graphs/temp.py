...
df['cost_change_start'] = pd.to_numeric(df['cost_change_start'], errors='coerce')
...
df['team_rank'] = df.groupby(['team', 'position_name'])['start_cost'].rank(method='average', ascending=False)
...
for i, pos in enumerate(positions):
    ...
    ax.set_title(f'{pos} - Initial Intra-Squad Hierarchy', fontweight='bold')
...
plt.suptitle('Predicting Minutes by Initial Intra-Squad Price Rank', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.show()
